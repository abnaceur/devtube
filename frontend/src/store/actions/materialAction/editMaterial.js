...
function uploadVedeo(dataHttp, token, dispatch) {
    ...
    axios.put(process.env.REACT_APP_API_URL + "/materials/upload/videos", dataHttp, {
        headers: {
...
export function editMyMaterial(data) {
    ...
    return async function (dispatch) {
        ...
        dataHttp.append('materialId', data.materialId);
        dataHttp.append('oldPhoto', data.oldPhoto);
        ...
        if (data.videos.length > 0)
            for (let i = 0; i < data.videos.length; i++) {
                ...
                dataHttp.append('views' + i, data.videos[i].views);
                dataHttp.append('_id' + i, data.videos[i]._id);
