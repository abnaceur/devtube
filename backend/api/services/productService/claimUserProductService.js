...
updateUserRequestNotification = (userId, data, requestId) => {
    return new Promise((resolve, reject) => {
        let notifictaion = new Notification({
            _id: new mongoose.Types.ObjectId,
            userId: userId,
...
async function requestUserCourse(res, data, userId) {
    // CHECK IF TH COURSE EXISTS
    let courseExit = await getCourseByName(data.title);
    ...
    if (!courseExit) {
        let claim = new Claim({
            ...
            claimCategory: "Course",
            claimTitle: data.title,
            claimDescription: data.description,
            claimPhoto: "",
            claimPrice: data.price
        })
        claim.save()
            .then(async results => {
                if (await updateUserRequestNotification(userId, data, results._id)) {
